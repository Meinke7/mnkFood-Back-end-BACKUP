...
class RequestsContoller {
  async create(request, response) {
    const { title, description, price, category, amount, plate_id, user_id } =
      request.body;
    ...
    if (checkIfExist) {
      await knex("requests").where({ plate_id }).update({ amount: amount });
    } else {
  ...
  async delete(request, response) {
    ...
    if (plate_id == 0) {
      ...
      await knex("requests").where({ plate_id }).where({ user_id }).delete();
    }
